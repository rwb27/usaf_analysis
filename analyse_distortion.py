...
if __name__ == '__main__':
	...
	dirs = [os.path.join(datasets, d) for d in os.listdir(datasets)]# if "pilens" in d]
	with PdfPages(os.path.join(here,"distortion_summary_radial.pdf")) as summary_radial, \
		PdfPages(os.path.join(here,"distortion_summary_images.pdf")) as summary_images, \
		PdfPages(os.path.join(here,"distortion_summary_grids.pdf")) as summary_grids:
		for dir in dirs:
			...
			with PdfPages(os.path.join(dir, "distortion_analysis.pdf")) as pdf:
				...
				summary_radial.savefig(figs[4])
				summary_grids.savefig(figs[0])
				summary_images.savefig(figs[3])
				for f in figs:
					pdf.savefig(f)
					plt.close(f)
	#plt.show()
